    ...
    public class EqualizerControlViewModel : BindableBase
    {
            ...
            while (showSaveDialog)
            {
                if ((bool)dlg.ShowDialog())
                {
                    ...
                    else
                    {
                        ...
                        {
                            AppLog.Error("An error occurred while saving preset to file '{0}'. Exception: {1}", dlg.FileName, ex.Message);
        ...
        private async Task DeletePresetAsync()
        {
                ...
                ResourceUtils.GetString("Language_No")))
            {
                ...
                {
                    AppLog.Error("An error occurred while deleting preset '{0}'. Exception: {1}", this.SelectedPreset.Name, ex.Message);
