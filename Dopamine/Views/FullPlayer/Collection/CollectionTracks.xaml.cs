    ...
    public partial class CollectionTracks : CommonViewBase
    {
        ...
        private async void DataGridTracks_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                await this.playbackService.EnqueueAsync(dg.Items.OfType<TrackViewModel>().ToList(), (TrackViewModel)dg.SelectedItem, PlaybackQueueContext.Songs, null);
            }
        }
        ...
        protected async override Task ActionHandler(Object sender, DependencyObject source, bool enqueue)
        {
            ...
            {
                AppLog.Error("Error while handling DataGrid action. Exception: {0}", ex.Message);
            }
        }
        ...
        protected override async Task ScrollToPlayingTrackAsync(object sender)
        {
            ...
            {
                AppLog.Error("Could not scroll to the playing track. Exception: {1}", ex.Message);
            }
        }
        ...
        protected override void ViewInExplorer(object sender)
        {
            ...
            {
                AppLog.Error("Could not view track in Windows Explorer. Exception: {0}", ex.Message);
            }
        }
